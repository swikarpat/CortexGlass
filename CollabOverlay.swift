...
// 2. Hardware Ghost HUD Panel (Excluded from WebRTC / Screen Capture)
class CollabPanel: NSPanel {
    ...
    init(rect: NSRect) {
        ...
        sharingType = .none // Excluded from Zoom, Teams, Google Meet, and browser screen sharing
        level = .floating
...
class AppDelegate: NSObject, NSApplicationDelegate, WKNavigationDelegate, SCStreamOutput {
    ...
    // 4-Mode Cognitive Assistant Engine
    enum Mode {
    ...
    // Continuous Rolling Audio Tap & VAD State
    ...
    let audioQueue = DispatchQueue(label: "com.swikar.audio.q", qos: .userInitiated)
    ...
    // Strips extraneous Gemini UI elements for a clean teleprompter view
    ...
    // Option + I : Toggle mouse interactivity, dragging, and focus
    ...
    // Option + R : Silent DOM-level reset without login redirects
    func resetRound() {
    ...
    // Option + A : Toggle live system audio capture via ScreenCaptureKit
    func toggleAudio() {
    ...
    func stream(_ stream: SCStream, didOutputSampleBuffer sb: CMSampleBuffer, of type: SCStreamOutputType) {
        ...
        if rms >= 0.015 {
            ...
            // SLIDING BUFFER TRIGGER: If speech continues non-stop for >= 18s without pausing, flush and transcribe
            if speechDuration >= 18.0 {
                let s = audioSamples
                isTranscribing = true
                speechDuration = 0
                if audioSamples.count > 32000 { audioSamples = Array(audioSamples.suffix(32000)) } // Retain 2s for context
            ...
            // AGGRESSIVE TURN COMPLETION: 0.9s pause detects end-of-question immediately
    ...
    func processWhisper(samples: [Float]) {
        ...
        // Scaled to 6 threads on Apple Silicon for sub-250ms transcription
        p.arguments = ["-m", "/usr/local/bin/whisper-engine/ggml-base.en.bin", "-f", "/tmp/collab_interviewer.wav", "--no-timestamps", "-nt", "-t", "6"]
        let pipe = Pipe()
        ...
        if c.count >= 6 {
            print("🎯 Spoken Question: \"\(c)\"")
            // MULTIMODAL AUDIO-VISUAL FUSION: Grab current screen text and fuse with spoken words
            Task {
    ...
    // High-Speed Silent Screen OCR Helper
    ...
    // Pure Teleprompter Prompt Engine (First-Person Verbatim Outputs)
    func buildPrompt(spokenInput: String, screenContext: String) -> String {
        let vault = loadContextVault()

        let baseInstructions = """
        ROLE & TONE:
        You are my personal real-time teleprompter in a live high-stakes interview.
        Write in FIRST PERSON ("I", "my team", "we") as a senior Staff AI/ML & Distributed Systems Architect.
        Write clean, direct, conversational English for me to glance at and READ OUT LOUD VERBATIM.
        NEVER write meta-commentary, introductory remarks, or academic lectures.

        CANDIDATE GROUND TRUTH CONTEXT (My career history, projects, and numbers):
        \(vault)

        FALLBACK POLICY:
        For past projects, technical vetoes, or career decisions, answer STRICTLY using the ground truth above.
        For new coding challenges, live debugging, or external systems design questions not covered above, seamlessly leverage your full Staff-level knowledge to provide the optimal solution in my voice.
        """
        ...
        case .coding:
            ...
            CURRENT SCREEN CODE BUFFER (OCR from CoderPad/IDE):
            \(screenContext)

            OUTPUT EXACTLY IN THIS TELEPROMPTER FORMAT:
            ### 1. SAY THIS IMMEDIATELY (Verbatim Script — Read out loud)
            2 to 3 natural spoken sentences directly addressing the interviewer, pointing out the core algorithmic approach or the exact bug/race condition, and setting up the code change.
            ...
            ### 4. SUBTLE TRAP / EDGE CASE TO MENTION
            1 sentence calling out a senior consideration (e.g., concurrency deadlock, off-by-one, null safety, or memory leak).
            """
        ...
        case .systemDesign:
            ...
            SYSTEM DESIGN SCENARIO (Spoken or Whiteboard):
            "\(spokenInput)"

            WHITEBOARD / SCREEN TEXT:
            \(screenContext)

            OUTPUT EXACTLY IN THIS TELEPROMPTER FORMAT:
            ### 1. SAY THIS IMMEDIATELY (Verbatim Opener — Read out loud)
            2 to 3 sentences clarifying scale constraints (QPS/TPS, p99 latency SLA), state requirements, and primary bottleneck.

            ### 2. MONOSPACE ASCII WHITEBOARD (Optimized for Excalidraw)
            ...
            ### 4. CONVERSATIONAL CHECK-IN (1 sentence)
            A natural prompt throwing the ball back to the interviewer to explore deep dives.
            """
        ...
        case .projectDeepDive:
            ...
            PROJECT RETROSPECTIVE QUESTION:
            "\(spokenInput)"
        ...
        case .behavioral:
            ...
            OUTPUT EXACTLY IN THIS TELEPROMPTER FORMAT:
            ### 1. SAY THIS IMMEDIATELY (Verbatim STAR Story — Read out loud)
    ...
    // Manual OCR Capture (Option + O / Option + P)
    ...
    // Carbon Hotkeys & Swallowing Engine
    func setupHotkeys() {
    ...
    func handleKey(_ id: UInt32) {
        ...
        case 18: resetRound()
        case 19: sendToGemini("Summarize your previous response into 2 ultra-concise, high-impact bullet points for an immediate 15-second verbal answer right now.")
